...
import { JAVA_CERTIFICATE_DATA } from '@/utils/constants';
...
import { JavaCertificate } from '@/utils/types';
import JavaCertificateDetailModal from '@/components/modals/JavaCertificateDetailModal';
...
// 1. Sidebar Component
// Reused similar structure but with Code icon for Java
const Sidebar: React.FC<SidebarProps> = ({ activeView, onViewChange, counts }) => {
    const navItems = [
        { id: "all", label: "All Certificates", icon: CommandLineIcon, color: "text-blue-500" },
...
const JavaCertificateTab: React.FC = () => {
    ...
    const filteredData = useMemo(() => {
        ...
        if (searchQuery) {
            ...
            result = result.filter(r =>
                r.aliasName.toLowerCase().includes(query) ||
    ...
    const getTitle = () => {
        switch (activeView) {
            case "expiring": return "Expiring Java Certificates";
            case "expired": return "Expired Java Certificates";
            case "prod": return "Production Java Certificates";
            case "dev": return "Development Java Certificates";
            case "test": return "Test Java Certificates";
            case "qa": return "QA Java Certificates";
            case "lab": return "Lab Java Certificates";
            default: return "Java Certificate Inventory";
        }
    };
                    ...
                    >
                        <td className="px-5 py-3">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="font-medium text-gray-900 text-sm">{row.aliasName}</span>
                            </div>
                        </td>
                        <td className="px-5 py-3">
                        ...
                        <td className="px-5 py-3 text-sm text-gray-600 truncate max-w-[200px]" title={row.javaHome}>
                            {row.javaHome}
                        </td>
